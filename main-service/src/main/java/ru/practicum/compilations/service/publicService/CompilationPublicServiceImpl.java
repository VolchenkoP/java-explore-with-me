...
public class CompilationPublicServiceImpl implements CompilationPublicService {
    ...
    public List<CompilationResponse> getCompilations(boolean pinned, int from, int size) {
        ...
        List<EventByCompilationId> eventsByCompIdIn = eventByCompilationRepository
                .findEventsByCompilationIdIn(compilationMap.keySet());

        Map<Integer, List<EventResponseShort>> eventShortListByCompId = new HashMap<>();

        for (EventByCompilationId eventByCompId : eventsByCompIdIn) {
        ...
        for (Compilation compilation : compilationMap.values()) {
            List<EventResponseShort> events = eventShortListByCompId.get(compilation.getId());
            if (events == null) {
                events = List.of();
            }
            CompilationResponse response = compilationMapper.toResponse(compilation);
